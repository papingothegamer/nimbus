...
### 1.1 JSON Structure
The `.nmbs` file contains the complete state of the `ProjectSystem`, serialized hierarchically.
...
Writing directly to the target `.nmbs` file is dangerous. If the application crashes halfway through the write operation, the file is corrupted.

### 2.1 The Atomic Save Process
Nimbus uses an atomic saving strategy:
1.  Serialize the project state into a temporary file in the same directory (e.g., `project_name.nmbs.tmp`).
...
*   **Storage:** Autosaves are written to a hidden `.backup` folder within the project directory, timestamped (e.g., `autosave_20260614_1430.nmbs`).
...
2.  It compares the timestamp of the latest autosave against the last user-saved `.nmbs`.
...
*   When importing a sample (e.g., a kick drum WAV) into the timeline, Nimbus defaults to copying that file into a local `/Audio` folder next to the `.nmbs` file.
